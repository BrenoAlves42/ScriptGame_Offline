...
function fetchDados(){
    ...
    if (savedData) {
        ...
        let values = savedData.split('|');

       
        currentCheckpoint = values[0]; 
        currentMapa = values[1]; 
        vidasRestantes = values[2];      
        hpRestante = values[3];        
...
function atualizarSave(){

    let savedData = localStorage.getItem('save');

    let currentCheckpointLocal = 0;
    let currentMapaLocal = 0; 
    ...
    if (savedData) {
        ...
        currentCheckpointLocal = values[0]; 
        currentMapaLocal = values[1];      
   ...
   switch (mapaAtual) {
    case 4:
        if(currentCheckpointLocal < 4){
            currentCheckpointLocal = 4;
        }
        break;
    case 8:
        if(currentCheckpointLocal < 8){
            currentCheckpointLocal = 8;
        }
        break;
    case 12:
        if(currentCheckpointLocal < 12){
            currentCheckpointLocal = 12;
        }
    default:
        break;
    }
    ...
    console.log("Mapa no save: "+currentMapaLocal+"// Mapa atual: "+mapaAtual);
    if(parseInt(currentMapaLocal) < parseInt(mapaAtual)){
        currentMapaLocal = parseInt(mapaAtual);
    }else{
        currentMapaLocal = currentMapa;
    }

    let saveData = `${currentCheckpointLocal.toString().padStart(2, '0')}|` +
               `${currentMapaLocal.toString().padStart(2, '0')}|` +
               `${vidasRestantes.toString().padStart(2, '0')}|` +
               `${hpRestante.toString().padStart(2, '0')}`;
...
function updateSaveMorreu(){
    let savedData = localStorage.getItem('save');

    let currentCheckpointLocal = 0;
    let currentMapaLocal = 1; 
    ...
    if(parseInt(currentCheckpointLocal) > 0){
        currentMapaLocal = currentCheckpointLocal;
    }
